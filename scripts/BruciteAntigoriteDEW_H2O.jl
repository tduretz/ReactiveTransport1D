...
function Speciation()
    ...
    df = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/MatrixBruciteAntigoriteDEW_H2O.csv", DataFrame))
    species      = names(df)[2:end-2] # Read column labels

    # Thermodynamic parameters for activity coefficients
    Aᵧ    = 0.7879
    Bᵧ    = 0.3711
    bdot  = [Aᵧ, Bᵧ]  # B-dot coefficients Aᵧ and Bᵧ at 300 C and 5 kbar
