...
function Speciation(logaoxides, T_calc, P)
    ...
    df = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/MatrixOlivineAntigorite_500C_dissolution_Fe_H2-O2.csv", DataFrame))
    ...
    Atg           = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/Atg_dissolution_500C.csv", DataFrame))
    LogK_Atg      = collect(Atg[:,2])                  # Log K values for MgOH dissociation
    Fa            = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/Fa_dissolution_500C.csv", DataFrame))
    LogK_Fa       = collect(Fa[:,2])                   # Log K values for MgOH dissociation
    ...
    b[3] = Itp1D_rev_scalar1(P_logK, LogK_Fo, 1000*P)          # Interpolate log Ks for forsterite dissolution
    b[4] = Itp1D_rev_scalar1(P_logK, LogK_Atg, 1000*P)         # Interpolate log Ks for antigorite dissolution
    b[5] = Itp1D_rev_scalar1(P_logK, LogK_Fa, 1000*P)          # Interpolate log Ks for fayalite dissolution
    aFo = 0.90
    aFa = 0.1
    b[3] = b[3] - log10(aFo)                                   # Correction of log K for diluted forsterite
    b[5] = b[5] - log10(aFa)                                   # Correction of log K for diluted fayalite
    b[6] = -25.0
...
function GetChemicalPotentials(X, Xoxides, data, T_calc, P, sys_in)
    ...
    return out.Gamma[1], out.Gamma[2], out.Gamma[3], out.Gamma[4], out.Gamma[6]
