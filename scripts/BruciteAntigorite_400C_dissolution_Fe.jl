...
function Speciation(logaoxides, T_calc)
    ...
    df = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/MatrixBruciteAntigorite_400C_dissolution_SiO2(aq)_wo_Mg(OH)2_Fe.csv", DataFrame))
...
function GetChemicalPotentials(X, Xoxides, data, T_calc, P, sys_in)
    ...
    @printf("The number of phases are %01d and the number of solid solutions are %01d\n",length(out.ph), length(out.sol_name))
