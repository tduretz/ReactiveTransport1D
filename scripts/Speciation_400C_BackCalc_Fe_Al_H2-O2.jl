...
function Speciation(logaoxides, T_calc, P)
    ...
    df = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/Matrix_BackCalc_Fe_Al_H2-O2.csv", DataFrame))
    ...
    HAlO2         = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/HAlO2_dissociation_400C.csv", DataFrame))
    LogK_HAlO2    = collect(HAlO2[:,2])                # Log K values for Al(OH)3 dissociation
    ...
    b[6] = logaoxides[4]
    b[7] = logaoxides[5]
    b[8] = Itp1D_rev_scalar1(P_logK, LogK_MgCl, 1000*P)        # Interpolate log Ks for MgCl+ dissociation constant
    b[9] = Itp1D_rev_scalar1(P_logK, LogK_HCl, 1000*P)         # Interpolate log Ks for HCl dissociation constant
    b[10] = Itp1D_rev_scalar1(P_logK, LogK_H2O, 1000*P)        # Interpolate log Ks for H2O dissociation constant
    b[11] = Itp1D_rev_scalar1(P_logK, LogK_MgOH, 1000*P)       # Interpolate log Ks for MgOH+ dissociation constant
    b[12] = Itp1D_rev_scalar1(P_logK, LogK_FeCl2, 1000*P)      # Interpolate log Ks for FeCL2 dissociation constant
    b[13] = Itp1D_rev_scalar1(P_logK, LogK_FeCl, 1000*P)       # Interpolate log Ks for FeCl+ dissociation constant
    b[14] = Itp1D_rev_scalar1(P_logK, LogK_FeOH, 1000*P)       # Interpolate log Ks for FeOH++ dissociation constant
    b[15] = Itp1D_rev_scalar1(P_logK, LogK_H2_H2O, 1000*P)     # Interpolate log Ks for H2O dissociation constant with H2(aq) and O2(g)
    b[16] = Itp1D_rev_scalar1(P_logK, LogK_HAlO2, 1000*P)      # Interpolate log Ks for HAlO2(aq) dissociation constant
...
function GetChemicalPotentials(X, Xoxides, data, T_calc, P, sys_in)
    ...
    return out.Gamma[1], out.Gamma[4], out.Gamma[3], out.Gamma[6], out.Gamma[5], out.Gamma[2]
...
µ_SiO₂, µ_FeO, µ_MgO, µ_H₂O, µ_O, µ_Al₂O₃ = GetChemicalPotentials(X_comp, Xoxides, data, T_calc, P, sys_in)
...
S0_Al₂O₃   = 129.855             # Entropy of Al2O3 at 298 K and 1 bar (J/mol/K)
...
G0_Al_lt   = (CSV.read("/Users/guillaumesiron/Documents/Julia_scripts/ReactiveTransport1D/data/G0_Al3+_400C.csv", DataFrame))
...
G0_Al      = collect(G0_Al_lt[:,2])                          # Gibbs free energies for Al3+ at different pressures (1 to 25 kbar)
...
G_Al⁰      = Itp1D_rev_scalar1(P_var, G0_Al, 1000*P)         # G0 for Al3+ at the pressure of interest
G_O₂⁰      = Itp1D_rev_scalar1(P_var, G0_O2, 1000*P)         # G0 for O2(g) at the pressure of interest
@printf("Gibbs free energy of Mg2+ = %2.5e, SiO2(aq) = %2.5e, Fe2+ = %2.5e, O2(g) = %2.5e and Al3+ = %2.5e\n", G_Mg⁰, G_SiO₂⁰, G_Mg⁰, G_O₂⁰, G_Al⁰)
...
logAlH = (1000*µ_Al₂O₃ + S0_Al₂O₃*298.15 - 3000*µ_H₂O - 3*S0_H₂O*298.15 - 2*G_Al⁰) / (2.303 * R * (T_calc+273.15))
...
@printf("Log(aSiO2) = %2.5e, Log(fO2) = %2.5e, log(aFe2+/aH+) = %2.5e, log(aMg2+/aH+) = %2.5e and log((aAl3+)2/(aH+)6) = %2.5e\n", logSiO₂H, logO₂, logFeH, logMgH, logAlH)
logaoxides = [logMgH; logSiO₂H; logFeH; logO₂, logAlH]
